...
- **CI / GitHub Actions.** SHA-pin every third-party action (full commit SHA + trailing `# vN`), never a mutable tag; Dependabot's `github-actions` ecosystem keeps the pins current. Use Node-24 action majors (`actions/checkout@v5`, `actions/setup-node@v5` — v4 is deprecated Node 20). Stage *required* branch-protection checks behind their precondition: CodeQL becomes required only after its first green run on `main`; require-signed-commits only after the maintainer's gpg key is registered on GitHub (an unregistered key reads as unverified and blocks every PR — cf. stele `st-001`, atrium `at-033`).
